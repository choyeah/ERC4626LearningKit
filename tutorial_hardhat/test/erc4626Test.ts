...
describe("Start Example ERC4626 test", async () => {
  ...
  async function ERC4626Fixture() {
    ...
    await exampleERC20
      .connect(addr1)
      .approve(exampleERC4626.address, changeToEthers(100));
  ...
  describe("Test deploying", () => {
    it("Should deploy correctly", async () => {
      const {
        ...
        addr2,
        changeToEthers,
      } = await loadFixture(ERC4626Fixture);
  ...
  describe("Test deposit exampleERC4626", () => {
    it("Should deposit correctly for the Example ERC4626 Contract", async () => {
      ...
      const shares = await exampleERC4626.previewDeposit(
        changeToEthers(amount)
      );
      await expect(
        exampleERC4626
          .connect(addr1)
          .deposit(changeToEthers(amount), addr1.address)
      )
  ...
  describe("Check max withdraw & reddem exampleERC4626", () => {
    it("Should get max withdraw & redeem correctly for the Example ERC4626 Contract", async () => {
      ...
      maxWithdraw = expect(
        await exampleERC4626.maxWithdraw(addr1.address)
      ).to.equal(changeToEthers(amount));

      maxRedeem = expect(
        await exampleERC4626.maxRedeem(addr1.address)
      ).to.equal(changeToEthers(amount));
    });
  });
  ...
  describe("Check max withdraw exampleERC4626", () => {
    it("Should get max withdraw correctly for the Example ERC4626 Contract", async () => {
      ...
      console.log(
        ...
        await exampleERC20.balanceOf(addr1.address)
      );

      await exampleERC4626
        .connect(addr1)
        .deposit(changeToEthers(amount), addr1.address);
      console.log(
        "erc20 balance after deposit : ",
        await exampleERC20.balanceOf(addr1.address)
      );
      ...
      await exampleERC4626
        .connect(addr1)
        .withdraw(changeToEthers(amount), addr1.address, addr1.address);
